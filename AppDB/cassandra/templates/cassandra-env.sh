...
then
    JVM_OPTS="$JVM_OPTS -javaagent:/var/lib/appscale/AppDB/cassandra/cassandra/lib/jamm-0.2.5.jar"
fi
...
# note: added check to see if the JVM is 64-bit
if echo "$java_ver_output" | grep 64-Bit && [ "$JVM_VERSION" \> "1.7" ]; then
